    ...
    switch (event)
    {
        ...
        case BTA_DM_INQ_RES_EVT:
        {
           ...
           BTIF_TRACE_DEBUG3("%s() %s device_type = 0x%x\n", 
							FUNCTION__, bd2str(&bdaddr, &bdstr),p_search_data->inq_res.device_type)
#else
           BTIF_TRACE_DEBUG3("%s() %s device_type = 0x%x\n", __FUNCTION__, bd2str(&bdaddr, &bdstr),BT_DEVICE_TYPE_BREDR);
#endif
#endif
            bdname.name[0] = 0;
            ...
            if (p_search_data->inq_res.p_eir)
            {
                ...
                //BTIF_TRACE_DEBUG3("%s() EIR BTA services = %08X", __FUNCTION__,(UINT32)services);
