...
uint8_t userial_open(uint8_t port)
{
    ...
    if (bt_vnd_if)
    {
        ...
        printf("\n%s: %s : Calling vendor-specific part\n",__FILE__,__FUNCTION__);
    ...
    else
    {
        ...
        printf("\n %s: %d : userial_open: unable to open UART port \n",__FUNCTION__,__LINE__);
        return FALSE;
    }
#endif    
 ...
 * */
	...
	if (fd < 0) {
		printf("Failed to open Unix server socket");
		return -1;
	}
	...
	//strncpy(addr.sun_path, path, sizeof(addr.sun_path) - 1);
	strcpy(addr.sun_path, "/home/nikhil/Desktop/bt-server-bredr");

	if (connect(fd, (struct sockaddr *) &addr, sizeof(struct sockaddr_un)) < 0) {
          close(fd);
          printf("\n %s: %d connecting stream socket Nikhilesh \n",__FUNCTION__,__LINE__);
...
uint16_t  userial_read(uint16_t msg_id, uint8_t *p_buffer, uint16_t len)
{
    ...
    //ALOGE("[h4] recv len:%d-%d\n", total_len, recv_byte_total);
    return total_len;
}
...
uint16_t userial_write(uint16_t msg_id, uint8_t *p_data, uint16_t len)
{
    ...
    if (len < 0)
    {
        ALOGE("[h4] eeeeeeeeeeeeeeeeeeeeeeeeeeeeeee:%d-%d\n", len1,send_byte_total);
        return 0;
    }
    while(len != 0)
    {
        ...
        if (ret < 0)
        {
            ...
            ALOGE("[h4] eeeeeeeeeeeeeeeeeeeeeeeeeeeeeee111:%d-%d-%d-%d-%d-%d-%s-%d\n", ret,send_byte_total, 
                userial_cb.fd, len,total, errno, strerror(errno),retry);
            retry++;
            usleep(10000);
            continue;
        }
